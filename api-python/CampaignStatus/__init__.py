# Classic (function.json) HTTP function: GET /api/campaign/status?runId=...
import os, json
...
def _blob_service() -> BlobServiceClient:
    """Build a BlobServiceClient from UPLOADS_SAS_URL (account SAS)."""
...
def _map_runtime_to_state(runtime_status: str) -> str:
    # UI states: ValidatingInput|EvidenceBuilder|DraftCampaign|QualityGate|Completed|Failed
    ...
    if rs in ("Pending",):
        return "ValidatingInput"
    if rs in ("Running",):
        return "DraftCampaign"   # blob status.json is authoritative when present
    if rs in ("Completed",):
        return "Completed"
...
async def main(req: func.HttpRequest, starter: str) -> func.HttpResponse:
    ...
    if not run_id:
        return func.HttpResponse(
            json.dumps({"error": "Missing runId"}), status_code=400, mimetype="application/json"
        )
    ...
    # 2) Blob status.json (authoritative when present): results/campaign/**/<runId>/status.json
    try:
        cc = _blob_service().get_container_client(os.environ["CAMPAIGN_RESULTS_CONTAINER"])
        prefix = "results/campaign/"; chosen = None
