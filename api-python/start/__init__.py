...
async def campaign_start(req: func.HttpRequest, client: df.DurableOrchestrationClient) -> func.HttpResponse:
    ...
    page = (body.get("page") or "default").strip()
    row_count = int(body.get("rowCount") or 0)
    filters = body.get("filters")
    csv_sha256 = body.get("csv_sha256")
    requested_run_id = body.get("runId")  # let Durable choose if None
    company = body.get("company")  # optional passthrough
    ...
    resp = {"ok": True, "runId": instance_id}
