// /api/campaign-fetch/index.js 17-12-2025 v20
...
const VALID_MAP = Object.freeze({
  ...
  "strategy_v2/campaign_strategy.json": "strategy_v2/campaign_strategy.json",
...
const SECTION_KEYS = [ "executive_summary", "go_to_market", "offering", "sales_enablement", "proof_points" ];
...
module.exports = async function (context, req) {
  ...
  if (prefixOverride) {
    ...
    base = canonicalPrefix({ userId: "anonymous", page, runId });
  }

  base = base.replace(/^results\//, "").replace(/^\/+/, "");
  ...
  if (!(await statusCheck.exists()) && !prefixOverride) {
    context.res = {
      status: 404,
      headers: { ...H, "content-type": "application/json" },
      body: { error: "run_not_found", message: "No status.json found for this runId at the derived prefix." }
    };
    return;
  }
  ...
  if (fileKey !== "sections" && fileKey !== "section") {
    ...
    const shouldRetry = fileKey === "campaign" || fileKey === "outline" || fileKey === "evidence_log";
    const ok = await existsWithTinyRetry(blob, shouldRetry);

    if (!ok) {
      context.res = { status: 404, headers: { ...H, "content-type": "application/json" }, body: { error: "not_found" } };
      return;
    }
    ...
    // ---------- STRICT CONTRACT CHECK (campaign.json ONLY) ----------
    
    return;
  }
if (fileKey === "campaign" && parsed && typeof parsed === "object") {
      const actualKeys = Object.keys(parsed);
      const { missing_keys, unexpected_keys } = diffKeys(SECTION_KEYS, actualKeys);
