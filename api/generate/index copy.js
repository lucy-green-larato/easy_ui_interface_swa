...
// ---- Engagement request/response contracts (local-only, suite-safe) ----
const CallScriptReq = z.object({
  kind: z.literal("call-script"),
  basePrefix: z.string().optional(),
  templateMdText: z.string().optional(),
  variables: z.object({
    product: z.string().min(1),
    buyerType: z.string().min(1),         // canonical buyer id
    mode: z.enum(["direct", "partner"]),
    tone: z.string().optional(),
    script_length: z.union([z.string(), z.number()]).optional()
  }).passthrough()
});

const CallFollowupReq = z.object({
  kind: z.literal("call-followup"),
  basePrefix: z.string().optional(),
  variables: z.object({
    seller_name: z.string().optional(),
    seller_company: z.string().optional(),
    prospect_name: z.string().optional(),
    prospect_role: z.string().optional(),
    prospect_company: z.string().optional(),
    tone: z.string().optional(),
  }).passthrough(),
  scriptMdText: z.string().optional(),
...
module.exports = async function (context, req) {
  ...
  // Enforce rate limit for POST only (adjust if you want GET too)
  if (req.method === "POST" && !rateLimitCheck(req)) {
