...
module.exports = async function (context, queueItem) {
  ...
  let prefix = normalizePrefix((msg && msg.prefix) || (runId ? `runs/${runId}/` : ""));
  const page = (msg && msg.page) || "campaign";

  if (!op || !runId || !prefix) {
    context.log.warn("[router] invalid payload", { type: typeof queueItem, sample: String(queueItem).slice(0, 200) });
    return;
  }
  ...
  try {
    ...
    if (op === "afteroutline") {
      ...
      if (!status0.markers.sectionsEnqueued) {
        ...
        status0.history.push({ at: nowISO(), phase: "Router", op: "afteroutline→sections", count: SECTION_KEYS.length });
        await saveStatus();
      ...
      if (!status0.markers.assembleEnqueued) {
        ...
        status0.history.push({ at: nowISO(), phase: "Router", op: "afteroutline→assemble" });
        await saveStatus();
    ...
    try {
      ...
      cur.history.push({ at: nowISO(), phase: "Router", op, error: String(err?.message || err) });
