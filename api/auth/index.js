// CommonJS (matches your /api package.json)
...
function principalFromCookie(req) {
  ...
  // Return a SWA-compatible principal
  return {
...
module.exports = async function (context, req) {
  ...
  // /.auth/login/* -> set dev cookie and redirect back
  ...
  // /.auth/logout -> clear cookie and redirect back
  ...
  // Optional: /.auth/refresh -> just return current principal
  ...
  // Anything else under /.auth -> 404 JSON (not HTML)
  context.res = {
    status: 404,
    headers: { "Content-Type": "application/json" },
